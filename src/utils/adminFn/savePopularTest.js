...
export async function savePopularTests(popularTestdata) {
    ...
    try {
        ...
        await sequelize.query(
            `DELETE FROM medibridge.popular_test WHERE id > $1`,
            { bind: [positions.length], transaction: tx }
        );
