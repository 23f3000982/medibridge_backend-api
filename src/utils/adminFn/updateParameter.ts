 ...
 */
export async function updateParameter(data: any) {
    ...
    try {
      ...
      `,
        ...
        const existing = rows[0] as { parameter_id: number; name: string; parameter_code: string }; // <-- SELECT returns an array
        if (!existing) {
...
export async function deleteParameter(data: any) {
