...
import { Image } from "../../constantTypes.js";

export async function imageToBlurhash(url: string): Promise<string> {
...
export async function updateBlurHash(): Promise<void> {
    console.log("🔄 Updating blurhash for all images...");
    const allImages: Image[] = await getAllImages(true);

    for (const image of allImages) {
        try {
            const blurhash: string = await imageToBlurhash(image.link);
