...
testRouter.use('/', async (req, res) => {
    ...
    if (slug) {
        const specificTest = allTests.find((test) => test.slug === slug);
        if (specificTest) {
...
export function toSendableTest(test) {
    const { testId, name, slug, basePrice, crelioId, departmentCode, tat, sampleId, sampleInfo, modelImage, icon, description, fastingRequired, parameters, createdAt, updatedAt, parameterInfo } = test;
